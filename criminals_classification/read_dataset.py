...
def read_charge_features():
    ...
    charge_features={} 
    f=open('criminals_classification/charges_features.txt')
    s=f.readline()
    headers=list(map(str.strip,s.split(',')))[1:]
    while True:
        l=f.readline()
        if l=='': break
        l=l.split(',')
...
def read_dataset():
    ...
    file_name="criminals_classification/compas-scores.csv"
    f=open(file_name)
    data=[]
    line_no=0
    cols=0
    ...
    for r in data[1:]:
        try:
            records.append(np.array(clean_record(r,nrs,head)))
